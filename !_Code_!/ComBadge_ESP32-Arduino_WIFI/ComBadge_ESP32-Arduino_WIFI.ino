...
/* ComBadge mk1 Arduino C++
  ...
  /!\ Unable to compile due to wake_on_touch not working...
...
#include "Arduino.h"
//#include "SoftwareSerial.h"           // Stopped working ~20/07/23 ~ ?????
...
#define touchPin T5   // GPIO12   Wake On Touch       Yellow (no plastic sleve on Dupont)
...
void wake_from_touch() {
  ...
  if ((ESP_SLEEP_WAKEUP_TOUCHPAD) && (touch_counter == false) && (wake_state == false))       // 25/07/23 not working?? re-write with 'case' from example & add individual functions for Shift Key and Main Tap
  {
...
void combadgeap()
{
  ...
  if (!wifiManager.startConfigPortal("ComBadge_Setup", "password")) {         // WiFi SSID & Password
  ...
  Serial.println("Connection Established...");                                //if you get here you have connected to the WiFi
  badgeid();

                                                                               //read updated parameters - WIP
...
void setup() {
  Serial.begin(115200);                                       //Serial on USB - Note #NOT default 9600
...
// Main Functions:
  ...
  if (touch_counter == 1) {       // Combadge Awake and Listening
    ...
    // To be Added from MQTT_TEST_SKETCH
  }

  
  if (touch_counter == 2) {       // Deactivate ComBadge
